...
type ChapterDAO struct{ db *gorm.DB }

func NewChapterDAO(db *gorm.DB) *ChapterDAO { return &ChapterDAO{db: db} }

func (d *ChapterDAO) ListByBook(ctx *gin.Context, bookId int64) ([]models.BookChapter, error) {
    var chapters []models.BookChapter
    err := d.db.WithContext(ctx).Where("book_id = ?", bookId).Order("id ASC").Find(&chapters).Error
    return chapters, err
}
